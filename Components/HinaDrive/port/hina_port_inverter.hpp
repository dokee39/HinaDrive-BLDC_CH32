...
{
    *hardware_.ppwm_duty_reg[phase] = (uint16_t)(duty * hardware_.duty_max);
}

inline void Inverter::portSetDuty(float duty_a, float duty_b, float duty_c) 
{
    *hardware_.ppwm_duty_reg[ePhaseA] = (uint16_t)(duty_a * hardware_.duty_max);
    *hardware_.ppwm_duty_reg[ePhaseB] = (uint16_t)(duty_b * hardware_.duty_max);
    *hardware_.ppwm_duty_reg[ePhaseC] = (uint16_t)(duty_c * hardware_.duty_max);
}

inline float Inverter::portGetDuty(ePhase phase)
{
    return ((float)*hardware_.ppwm_duty_reg[phase] / (float)hardware_.duty_max);
}
}
